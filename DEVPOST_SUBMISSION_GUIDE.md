...
*Democratizing farmland due diligence with zero-NIR visible spectrum computer vision, live global soil telemetry, and Claude 3.5 Sonnet to eliminate fertilizer runoff and de-risk agricultural investments.*
...
* **Agrometeorology Telemetry Engine:** Python `urllib` streaming from Open-Meteo and ISRIC SoilGrids v2.0 with automatic retry and zero external API keys. Computes a localized Drought Vulnerability Index ($DVI$).
* **Cognitive Diligence Engine:** Anthropic Claude 3.5 Sonnet (`claude-3-5-sonnet-20241022`) executing deep agronomic reasoning with a fully verified deterministic fallback engine when API keys are absent.
...
* **Zero API Key Out-of-the-Box Operation:** We wanted anyone, including hackathon judges running our code locally, to test the system with zero friction. We coupled Open-Meteo's open global weather grid with an agronomic deterministic synthesis mode that mirrors Claude 3.5 Sonnet's diagnostic logic.
...
### [2:00 – 2:35] Claude 3.5 Sonnet Reasoning & Fertilizer Prescriptions
* **Screen:** Scroll down to the **Agronomic Due Diligence Audit** and the **Precision Zonal Fertilizer Table**.
* **Voiceover:**  
  *"Simultaneously, TerraPulse queries live Open-Meteo agrometeorology and ISRIC SoilGrids—fetching multi-layer soil moisture at 0–7cm, 7–28cm, and rootzone, as well as soil pH and nitrogen reserves.*  
  *Anthropic's Claude 3.5 Sonnet synthesizes this vision data and soil chemistry into an objective Farmland Viability Score—here 68 out of 100 with an executive verdict.*  
